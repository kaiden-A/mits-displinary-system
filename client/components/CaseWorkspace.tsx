...
export default function CaseWorkspace({ id, roles, authType }: { id: string; roles: string[]; authType: "staff" | "pengawas" }) {
  ...
  return <div className="mx-auto max-w-[1240px]">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
      <div className="min-w-0">
        ...
        {activeTab === "investigation" ? <InvestigationPanel caseData={c} fields={fields} setFields={setFields} busy={busy} onSave={saveB02} /> : null}
        {activeTab === "documents" ? <DocumentsPanel caseData={c} requiredDocs={requiredDocs} docFields={docFields} setDocFields={setDocFields} meetingFields={meetingFields} setMeetingFields={setMeetingFields} busy={busy} onSaveDocument={saveDocument} onSaveMeeting={saveMeeting} /> : null}
...
function InvestigationPanel({ caseData, fields, setFields, busy, onSave }: { caseData: CaseDetail; fields: Record<string, string>; setFields: (value: Record<string, string>) => void; busy: string; onSave: () => void }) {
  const canAdd = ["REPORTED", "INVESTIGATING"].includes(caseData.status);
  return <div className="space-y-5"><Card className="p-5"><SectionTitle title="Borang siasatan B02" description="Setiap borang disimpan dengan nama dan peranan pengisi. Satu kes boleh mempunyai lebih daripada satu B02." />{caseData.b02_forms.length ? <div className="mb-5 space-y-2">{caseData.b02_forms.map((form) => <div key={form.id} className="rounded-lg border border-success-200 bg-success-50/60 px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-mono text-xs font-semibold text-success-800">B02-{form.id}</span><span className="text-xs font-semibold text-success-800">Disimpan {formatDateTime(form.filled_at)}</span></div><p className="mt-1 text-sm text-ink-800">Diisi oleh <strong>{form.fill_by}</strong> · {roleLabel(form.fill_role)}</p>{form.fields.isu ? <p className="mt-1 text-xs text-ink-600">Isu: {form.fields.isu}</p> : null}</div>)}</div> : <EmptyState icon="search" title="Belum ada laporan siasatan" description="Tambah B02 selepas perbualan, semakan saksi, atau pemeriksaan bukti." />}{canAdd ? <div className="border-t border-ink-100 pt-5"><p className="mb-4 text-sm leading-6 text-ink-700">Lengkapkan hanya maklumat yang diketahui. Medan saksi dan bahan sokongan adalah pilihan.</p><div className="grid gap-4 md:grid-cols-2">{B02_FIELDS.map(([key, label]) => <label key={key} className="block"><span className="mb-1.5 block text-sm font-semibold text-ink-800">{label}{["aduan", "tarikhAduan", "isu", "laporan"].includes(key) ? <span className="ml-1 text-danger-700">*</span> : null}</span>{["laporan", "punca", "penambahbaikan", "saksi", "bukti", "isu", "butiranPengadu"].includes(key) ? <textarea value={fields[key] || ""} onChange={(event) => setFields({ ...fields, [key]: event.target.value })} rows={key === "laporan" ? 4 : 2} className="w-full rounded-lg border border-ink-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-200" /> : <input type={key.toLowerCase().includes("tarikh") ? "date" : "text"} value={fields[key] || ""} onChange={(event) => setFields({ ...fields, [key]: event.target.value })} className="h-11 w-full rounded-lg border border-ink-200 bg-white px-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-200" />}</label>)}</div><Button className="mt-5" disabled={busy === "b02"} onClick={onSave}>{busy === "b02" ? "Menyimpan…" : "Simpan B02"}</Button></div> : null}</Card></div>;
}

function DocumentsPanel({ caseData, requiredDocs, docFields, setDocFields, meetingFields, setMeetingFields, busy, onSaveDocument, onSaveMeeting }: { caseData: CaseDetail; requiredDocs: { id: string; code: string; name: string; filled: boolean }[]; docFields: Record<string, string>; setDocFields: (value: Record<string, string>) => void; meetingFields: Record<string, string>; setMeetingFields: (value: Record<string, string>) => void; busy: string; onSaveDocument: (code: string, data: Record<string, string>) => void; onSaveMeeting: () => void }) {
...
function casePath(source: string, points: number) {
  ...
  return ["REPORTED", "RECORDED", ...(points >= 10 ? ["STUDENT_ACK", "ACTION_PREPARED", "PRINCIPAL_APPROVAL"] : []), "EXECUTED", ...(points >= 10 ? ["PARENT_NOTIFIED", "MEETING"] : []), "CLOSED"];
}
...
function requiredDocuments(caseData: CaseDetail) {
  ...
  if (caseData.points >= 10) codes.push("b05", "b06");
  if (caseData.points >= 30) codes.push("b08");
  if (caseData.offences.some((offence) => ["D02", "D03", "J01", "J06", "L09", "L13", "L15"].includes(offence.code))) codes.push("b07");
  codes.push("kad");
  return codes.map((code) => { const item = DOCS.find(([id]) => id === code); return { id: code, code: item?.[1] || code.toUpperCase(), name: item?.[2] || code, filled: existing.has(code) || (code === "b04" && !["REPORTED", "DISMISSED"].includes(caseData.status)) }; });
}
